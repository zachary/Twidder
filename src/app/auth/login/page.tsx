...
const page = () => {
    ...
    return (
        <React.Fragment>
            <p className={`w-full text-center text-3xl ${leagueSpartan.className} mb-10`}>Login</p>
            <form ref={formRef} onSubmit={handleLogin} className={`w-full h-full flex flex-col items-center justify-center gap-5`}>
                ...
                <button type="submit" className={`w-full text-lg rounded-sm text-white bg-cyan-500 py-2`}>Login</button>
            </form>
            <p className={`text-white text-sm ${roboto.className} mt-10`}>
                New Here ?
                <Link className={" ml-2 text-cyan-500 font-bold"} href="/auth/signup">
                    Signup
                </Link>
            </p>
        </React.Fragment>
